---
name: carousel-init
description: "Handles all pipeline preparation for a carousel run: selects the next drug×template combo, validates against the usage log, pulls the latest drug data, extracts the drug object, and writes the pipeline state file. Called only by carousel-master — never trigger directly."
version: "1.0.0"
author: rob
...
- Do not write to `carousel_usage_log.json` — that's Phase 5 in carousel-master
- Do not trigger any downstream subskills — carousel-master handles sequencing
- If `current-run/` already has files from a previous run, do not delete them — carousel-master handles cleanup decisions
